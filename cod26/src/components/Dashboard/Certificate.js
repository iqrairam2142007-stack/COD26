...
const v = { fontWeight: 700, color: "var(--text)" };
...
export default function Certificate({ progress, userName }) {
  ...
  return (
    <>
      ...
      <div style={{ height: 8, background: "var(--surface2)", borderRadius: 99, overflow: "hidden", maxWidth: 420, margin: "8px 0 18px" }}>
      ...
      {eligible
        ...
        : <div style={{ background: "var(--light)", border: "1px solid var(--border)", borderRadius: 12, padding: 14, marginBottom: 14 }}>
...
function CertCard({ dummy, name }) {
  ...
  return (
    <div style={{ background: "var(--surface)", border: "8px double var(--orange)", borderRadius: 16, padding: 36,
      textAlign: "center", maxWidth: 640, margin: "auto", position: "relative", opacity: dummy ? 0.92 : 1,
      boxShadow: "var(--glow)" }}>
      {dummy && <div style={{ position: "absolute", top: 14, right: -2, background: "var(--orange)", color: "#10131a",
      ...
      <div style={{ fontSize: "1.7rem", fontWeight: 800, color: "var(--text)", margin: "14px 0",
        borderBottom: "2px solid var(--border2)", display: "inline-block", padding: "0 24px 6px" }}>{name}</div>
